...
for k in range(0,5):  #len(stock_id)
    ...
    for i in tqdm.tqdm_notebook(range(0,len(train)-n)):#range(0,len(train)-n) 
        ...
        x_train.append(np.transpose(x_trainaddscalar))  #x_train.append(norm_data_xtrain.iloc[i:i+n]. values) 
        #修改
    ...
    for i in tqdm.tqdm_notebook(range(0,len(val)-n)):#range(0,len(train)-n) 
        ...
        x_val.append(np.transpose(x_valaddscalar))  #x_train.append(norm_data_xtrain.iloc[i:i+n]. values) 
        #修改
    ...
    for i in tqdm.tqdm_notebook(range(0,len(test)-n)): 
        ...
        x_test.append(np.transpose(x_testaddscalar)) #x_test.append(norm_data_xtest.iloc[i:i+n]. values) 
